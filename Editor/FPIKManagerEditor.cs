namespace FuzzPhyte.Utility.Animation.Editor
{
    ...
    public class FPIKManagerEditor:Editor
    {
        ...
        private SerializedProperty rightHandTarget, rightHandHint, rightHandWeightScale, rightHandMechRef;
        private SerializedProperty useLeftHandIK;
        private SerializedProperty leftHandTarget, leftHandHint, leftHandWeightScale, leftHandMechRef;
        private SerializedProperty handIKSpeed, reachProximityMax;
        private SerializedProperty rightArmConstraint, leftArmConstraint;
        ...
        private void OnEnable()
        {
            ...
            leftArmConstraint = serializedObject.FindProperty("LeftArmConstraint");
            ...
            handLFTOffsetPOS = serializedObject.FindProperty("UseLftRigOffsetPos");
            handLFTOffsetROT = serializedObject.FindProperty("UseLftRigOffsetRot");
            rightHandMechRef = serializedObject.FindProperty("RightHandRef");
            leftHandMechRef = serializedObject.FindProperty("LeftHandRef");
            ...
            if (ikActive.boolValue)
            {
                ...
                if (useHandIK.boolValue)
                {
                    ...
                    if (useRightHandIK.boolValue)
                    {
                        ...
                        {
                            EditorGUILayout.PropertyField(rightHandMechRef);
                        }
                    }
                    else
                    {
                    ...
                    if (useLeftHandIK.boolValue)
                    {
                        ...
                        if((HeadIKProvider)headIKMode.enumValueIndex == HeadIKProvider.AnimationRigging)
                        {
                            EditorGUILayout.PropertyField(leftArmConstraint);
                            EditorGUILayout.PropertyField(handLFTOffsetPOS);
                            EditorGUILayout.PropertyField(handLFTOffsetROT);
                        }
                        else
                        {
                            EditorGUILayout.PropertyField(leftHandMechRef);
                        }
                    }
                    else
                    {
